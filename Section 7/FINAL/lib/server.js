...
server.unifiedServer = function (req, res) {
  ...
  req.on('end', function () {
    ...
    // Check the router for a matching path for a handler. If one is not found, use
    // the notFound handler instead.
    var chosenHandler = typeof(server.router[trimmedPath]) !== 'undefined'
      ? server.router[trimmedPath]
      : handlers.notFound;

    // If the request is within the public directory use to the public handler
    // instead
...
server.processHandlerResponse = function (res, method, trimmedPath, statusCode, payload, contentType) {
  ...
  // Use the status code returned from the handler, or set the default status code
  // to 200
